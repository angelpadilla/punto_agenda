class InventarioPdf < Prawn::Document
  ...
  def head
    ...
    text "LISTA DE PRECIOS MAYOREO #{@timee.strftime("%d/%m/%Y")}", size: (@fontsize + 4), style: :bold
    move_down 10
  ...
  def item_rows
    [ [ "Existencia", "Medida", "Modelo", "Marca", "Precio Mayoreo" ] ] +
      @items.map do |item|
        alto = item.alto
        formatted_alto =
          if alto.nil?
        ...
        [
          ...
          item.mayoreo ? "$ #{item.mayoreo.round(1)}" : "-"
        ]
      end
  end
